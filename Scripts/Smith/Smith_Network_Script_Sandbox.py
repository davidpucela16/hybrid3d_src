...
def MyItAlg(Si_V, prob, tol_ref, max_iter, Cv, phi_bar):
    #Initial guess
    E=prob.Gij+prob.q_portion
    q=np.repeat(prob.K, prob.mesh_1D.cells)*(Cv-phi_bar)
    ...
    it=0
    tol=1
    while tol>tol_ref or it<max_iter:
        pdb.set_trace()
        
        s_prime=dir_solve(prob.A_matrix, -prob.B_matrix.dot(q[-1])-prob.I_ind_array)
        #Check
        r=Si_V.dot(q[-1])
        phi_coarse=s_prime+Si_V.dot(q[-1])
        s_prime[phi_coarse>1]=1-r[phi_coarse>1]
        s_prime[phi_coarse<0]=-r[phi_coarse>1]
        #calculate fluxes 
        q_prime=dir_solve(E, -prob.F_matrix.dot(Cv[-1]) - prob.D_matrix.dot(s_prime))
        q.append(q_prime)
        #new value of the intra-vascular concentration
        Cv_int=dir_solve(prob.I_matrix, -prob.aux_arr*q[-1]-prob.III_ind_array)
        #check
        Cv_int[Cv>1]=1
        Cv_int[Cv<0]=0
    
        q_prime_prime=dir_solve(E, -prob.F_matrix.dot(Cv_int) - prob.D_matrix.dot(s_prime))
        q.append(q_prime_prime)
        Cv.append(Cv_int)
        s.append(s_prime)
        
        it+=1
        tol=np.average(np.abs(q[-1]-q[-2])/np.abs(q[-1]))
        
    return s, q, Cv

#from scipy.sparse.linalg import bicgstab as solver
...
def IterativeAlgorithm2(Si_V, prob, tol_ref, max_iter, *initial_guess):
    ...
    while err>tol_ref or it>max_iter:
        ...
        pdb.set_trace()
        Cv=9
        while np.average(Cv)<0 or np.average(Cv)>1:
            Cv=solver(prob.I_matrix, -prob.H_matrix.dot(q_int)-prob.III_ind_array)
            q_int=(q+q_int)/2
...
if Computation_bool:
    ...
    #MyItAlg(Si_V, prob, 1e-5, 100, np.ones(len(net.pos_s)),np.ones(len(net.pos_s))*0.8)
    aa=time.time()
    IterativeAlgorithm2(Si_V, prob, 1e-5, 100)
    bb=time.time()
    cc=bb-aa
    pdb.set_trace()
