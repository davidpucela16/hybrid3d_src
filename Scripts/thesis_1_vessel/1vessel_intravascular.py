...
U = np.array([2]) #Upper guess from network flo, in micrometers per second
D = 2e3 # in micrometers^2 per second [Linninger]
K=10
L = np.array([10])
R=np.array([L[0]/50])
Pe = U*L/D

cells_array=np.array([5,10,30,80])

# Create the figure and axes objects
fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(20, 8))
for cells_1D in cells_array:
    ...
    axes[0].plot(s,sol, label='$L_v/h=${}'.format(cells_1D))
    axes[1].plot(s,sol_reac, label='$L_v/h=${}'.format(cells_1D))
    

axes[0].plot(s, analytical, label='analytical', linewidth=4)
box_string='Pe={}'.format(int(U*L/D))
axes[0].text(2,0.50, box_string, fontsize = 40, 
 bbox = dict(facecolor = 'white', alpha = 0.5))
axes[0].set_title('Advection - diffusion')
axes[0].set_ylabel('$C_v$', rotation=0, labelpad=20)
axes[1].plot(s, analytical_reac, label='analytical', linewidth=4)
axes[1].legend()
box_string='Da={:.1f}'.format(k*L[0]/D)
...
plt.savefig(path_output + '/Cv.svg')
# Show the plot
plt.show()
