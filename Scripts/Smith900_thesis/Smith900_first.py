...
#script='/home/pdavid/Bureau/Code/BMF_Code_clean/hybrid3d_src/Scripts/Smith/Smith_Network_Script_Reduced.py'
...
Computation_bool= not os.path.exists(os.path.join(path_matrices, 'sol_red_{}.npy'.format(gradient)))
rec_bool=False
...
if not flow_mine:
    ...
    Flow_rate=Flow_rate*1e6 #from nl/s to \mu m^3/s
    U = 4*Flow_rate/np.pi/diameters**2 #in \mu m/s
    ########################################################################
    ...
    if not os.path.exists(os.path.join(path_matrices, 'U_{}.npy'.format(gradient))):
        F=flow( bc_uid, bc_value, L, diameters, startVertex, endVertex)
        F.solver()
        Pressure=dir_solve(F.A, F.P)
        U=F.get_U()*factor_flow_mine
        np.save(os.path.join(path_matrices, 'U_{}.npy'.format(gradient)), U)
    else: 
        U=np.load(os.path.join(path_matrices, 'U_{}.npy'.format(gradient)))
        
        df = pd.read_csv(os.path.join(path_am, "VertexPressure.txt"), skiprows=2, sep="\s+", names=["length"])
        Pressure=np.ndarray.flatten(df.values)
    Flow_rate=np.abs(U*np.pi*diameters**2/4)
    ########################################################################
    #   THIS IS A CRUCIAL OPERATION
    ########################################################################
    print("Modifying edges according to pressure gradient")
    ...
    for i in range(len(edges)):
        if Pressure[endVertex[i]] > Pressure[startVertex[i]]:
            ...
            edges[i,0]=endVertex[i]
            edges[i,1]=startVertex[i]    
            U_mod[i]=-np.abs(U[i]) #After this U_mod=U otherwise there is an error 
    startVertex=edges[:,0].copy()
    endVertex=edges[:,1].copy()
K=diameters*factor_K
U_D=np.abs(U/D)
...
BCs_1D, lab=SetArtificialBCs(vertex_to_edge, 1,0, startVertex, endVertex)
...
if Computation_bool:
    ...
    a=time.time()
    sol_gcrotmk, _=sp.sparse.linalg.gcrotmk(A,-b,x0=InitialGuessSimple(Si_V, np.repeat(prob.K, net.cells), 0.1, np.ones(prob.S)), tol=1e-3, maxiter=1000, callback=callback)
    bb=time.time()
    
    prob.s=sol_gcrotmk[:prob.F]
    prob.Cv=sol_gcrotmk[prob.F:]
    prob.q=-sp.sparse.linalg.inv(prob.H_matrix).dot(prob.I_matrix.dot(prob.Cv)+prob.III_ind_array)
    np.save(os.path.join(path_matrices, 'sol_red_{}.npy'.format(gradient)), np.concatenate((prob.s, prob.q, prob.Cv)))
    t=bb-a
    arr_time.append(t)

    
#%% - Reduced
sol=np.load(os.path.join(path_matrices, 'sol_red_{}.npy'.format(gradient)))
prob.s=sol[:prob.F]
prob.Cv=sol[-prob.S:]
prob.q=sol[-2*prob.S:-prob.S]
# =============================================================================
...
corners=np.array([[0,0],[0,L_3D[0]],[L_3D[1],0],[L_3D[0], L_3D[1]]])*(shrink_factor)+(1-shrink_factor)*L_3D[0]/2
if simple_plotting:    
    aaz=VisualizationTool(prob, 2,0,1, corners, res)
    aaz.GetPlaneData(path_output_data)
    aaz.PlotData(path_output_data)
    res=300
    aaz2=VisualizationTool(prob, 2,0,1, corners, res)
    aaz2.GetPlaneData(path_output_data)
    aaz2.PlotData(path_output_data)
